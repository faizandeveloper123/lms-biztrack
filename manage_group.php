...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'AddGroup') {
        ...
        if ($name === '') { $error = 'Name is required.'; }
            ...
            $st = db_prepare("INSERT INTO `groups` (name) VALUES (?)");
    ...
    if ($action === 'UpdateGroup') {
        ...
        if ($name === '') { $error = 'Name is required.'; }
            ...
            $st = db_prepare("UPDATE `groups` SET name=? WHERE id=?");
    ...
    if ($action === 'DeleteGroup') {
        $id = (int) ($_POST['id'] ?? 0);
        $st = db_prepare("DELETE FROM `groups` WHERE id=?");
        $st->bind_param('i', $id);
        $st->execute();
...
if (isset($_GET['edit'])) {
    $eid = (int) $_GET['edit'];
    $st = db_prepare("SELECT * FROM `groups` WHERE id=?");
    $st->bind_param('i', $eid);
    $st->execute();
...
$rows = [];
$res = db_query("SELECT * FROM `groups` ORDER BY name");
...
<div class="main-content">
    <div class="container-fluid">
        <div style="display:flex; align-items:center; justify-content:space-between; padding:14px 4px;">
            <h3 style="font-size:18px; font-weight:800; color:#111827; margin:0;"><i class="fa fa-object-group"></i> Manage Groups / Shifts</h3>
            ...
            <div class="col-md-5">
                <div class="crud-card">
                    <h4 style="font-size:15px; font-weight:700; color:#111827; margin:0 0 14px;">
                        <?php echo $edit_row ? 'Edit Group' : 'Add New Group / Shift'; ?>
                    </h4>
                    <form method="post" action="manage_group.php">
                        <?php if ($edit_row): ?>
                        ...
                        <div class="form-group">
                            <label>Group / Shift Name</label>
                            <input type="text" name="name" class="form-control" required value="<?php echo e($edit_row['name'] ?? ''); ?>" placeholder="e.g. Morning Shift">
                        </div>
                        <button type="submit" class="btn btn-success" style="color:#fff;"><i class="fa fa-save"></i> <?php echo $edit_row ? 'Update' : 'Save'; ?></button>
                        <?php if ($edit_row): ?><a href="manage_group.php" class="btn btn-default">Cancel</a><?php endif; ?>
                    </form>
                </div>
            </div>
            <div class="col-md-7">
                <div class="crud-card" style="overflow-x:auto;">
                    <h4 style="font-size:15px; font-weight:700; color:#111827; margin:0 0 14px;">Group List (<?php echo count($rows); ?>)</h4>
                    <table class="table crud-table" style="width:100%; margin:0;">
                        <thead><tr><th>#</th><th>Name</th><th style="width:100px;">Actions</th></tr></thead>
                        <tbody>
                            <?php if (!$rows): ?>
                                <tr><td colspan="3" style="text-align:center; color:#95A5A6; padding:24px;">No groups yet.</td></tr>
                            <?php endif; ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="manage_group.php?edit=<?php echo $r['id']; ?>" class="btn btn-primary btn-xs" title="Edit"><i class="fa fa-pencil"></i></a>
                                    <form method="post" action="manage_group.php" style="display:inline;" onsubmit="return confirm('Delete this group?');">
