...
if ($q !== '') {
    ...
    if (preg_match($qr, $q)) {
        $stmt = db_prepare("SELECT s.student_id AS id, s.first_name, s.last_name, s.father_name, s.gr_no,
                        ...
                        WHERE s.gr_no LIKE ? OR s.roll_no LIKE ? OR s.student_id = ? OR CONCAT(s.first_name,' ',COALESCE(s.last_name,'')) LIKE ? OR COALESCE(s.father_name,'') LIKE ?
                        ORDER BY s.first_name LIMIT 12");
        $qlike = '%' . $q . '%';
        $exact = (int)$q;
        $stmt->bind_param('ssisss', $qlike, $qlike, $exact, $like, $like);
    } else {
        $stmt = db_prepare("SELECT s.student_id AS id, s.first_name, s.last_name, s.father_name, s.gr_no,
                        CONCAT(s.first_name, ' ', COALESCE(s.last_name,'')) AS name,
                        CONCAT(COALESCE(c.class_name,''), ' ', COALESCE(sec.section_name,'')) AS class_sec
                        ...
                        LEFT JOIN classes c ON c.class_id = s.class_id
                        LEFT JOIN sections sec ON sec.section_id = s.section_id
                        WHERE CONCAT(s.first_name,' ',COALESCE(s.last_name,'')) LIKE ? OR COALESCE(s.father_name,'') LIKE ? OR COALESCE(s.cell_no,'') LIKE ?
                        ORDER BY s.first_name LIMIT 12");
